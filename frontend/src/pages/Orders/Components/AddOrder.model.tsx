...
import { useCreateOrderMutation } from '../../../provider/queries/Order.query';
const AddOrderModel = ({ visible, setVisible }: any) => { 
    ...
    const validationSchema = yup.object({
        ...
        user: yup.mixed().required("Sight is required"),
        // items: yup.array().of(
        //     yup.object().shape({
                  ...
                  {({ values, setFieldValue, handleSubmit }) => (
                      <>
                          <form onSubmit={handleSubmit} className="w-full" >
                              <div className="mb-3">
                                  <label htmlFor="name">Sight <span className="text-red-500 text-sm">*</span> </label> 

                                  <Dropdown value={values.user} onChange={(e) => setFieldValue('user', e.value)} filterBy='name' options={data && data.users} filterPlaceholder='Search Sight By Name' optionLabel="user" placeholder="Select a Sight"
                                  emptyFilterMessage="No Sight Found"
                                  emptyMessage="You Have No Sight"
                              ...
                              <div className="mb-3">
                                  <label htmlFor="name">Items <span className="text-red-500 text-sm">*</span> </label>
                                        ...
                                        <div className="mb-3 flex justify-end">
                                                  <button type='button' onClick={() => { push({name:'',quantity:'',price:''})}} className='bg-black px-4  text-white py-2 rounded-sm'>Add +</button>
                                        </div>
                                                                                {
                                                  values.items.length>0 &&  values.items.map((_,i)=>{
                                                            ...
                                                            return<div className='w-full flex items-center justify-between gap-x-4' key={i}>
                                                                    ...
                                                                    {/* <ErrorMessage name={`items[${i}].name`} className='text-red-500 capitalize' component={'p'} /> */}
                                                                            ...
                                                                            <div className="w-1/2">
                                                                    <Field name={`items[${i}].quantity`} className="w-full my-2 border outline-none py-3 px-4" placeholder="Quantity" />
                                                                    {/* <ErrorMessage name={`items[${i}].name`} className='text-red-500 capitalize' component={'p'} /> */}
                                                                ...
                                                                <div className="w-1/2">
                                                                    <Field type="number" name={`items[${i}].price`} className="w-full my-2 border outline-none py-3 px-4" placeholder="Price" />
                              ...
                              <div className="flex justify-start">
                                  <Button unstyled  className="text-white px-5 rounded-sm bg-black py-3 text-center ">Update Material</Button>
                              </div>
